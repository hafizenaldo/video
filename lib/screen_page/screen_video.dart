...
import 'package:video/model/video_model.dart';
import 'package:video/screen_page/detail_video.dart';
...
class _PageVideoState extends State<PageVideo> {
  ...
  Future<void> _fetchVideoData() async {
    try {
      final response = await http.get(Uri.parse('http://192.168.18.8/video/video.php'));
      ...
      body: _isLoading
        ...
        itemBuilder: (context, index) {
          return GestureDetector(
            onTap: () {
              _navigateToVideoPlayerPage('http://192.168.18.8/video/clip/${_videoList[index].video}');
            },
            child: Card(
              margin: const EdgeInsets.all(16.0),
              child: ListTile(
                leading: Image.network(
                  'http://192.168.18.8/video/logo/${_videoList[index].thumbnail}',
                  width: 100,
                  fit: BoxFit.cover,
                ),
